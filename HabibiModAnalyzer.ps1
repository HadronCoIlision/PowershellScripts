...
$script:zmLog = Join-Path $env:TEMP 'ZachMacros.log'
function Write-ZMLog { param([string]$msg); $ts = Get-Date -Format 'HH:mm:ss'; "$ts  $msg" | Out-File $script:zmLog -Append -Encoding utf8 }
Write-ZMLog "=== START PID=$PID MacroMode=$MacroMode STA=$([Threading.Thread]::CurrentThread.GetApartmentState()) ==="
...
if ($MacroMode -and [Threading.Thread]::CurrentThread.GetApartmentState() -ne 'STA') {
    ...
    if ($scriptPath) {
        Write-ZMLog "STA relaunch: $psExe -File '$scriptPath'"
        Start-Process -FilePath $psExe -ArgumentList "-ExecutionPolicy Bypass -STA -NoProfile -File `"$scriptPath`" -MacroMode"
    }
    return
}
...
if (-not $MacroMode) {
    ...
    Write-ZMLog "Launcher: psExe='$psExe' scriptPath='$scriptPath'"

    if ($scriptPath) {
        Write-ZMLog "Local launch: -File '$scriptPath'"
        Start-Process -FilePath $psExe -WindowStyle Hidden -ArgumentList "-ExecutionPolicy Bypass -STA -NoProfile -File `"$scriptPath`" -MacroMode"
    } else {
        # Download ZachMacros to temp and launch it hidden in background
        $tmp = Join-Path ([IO.Path]::GetTempPath()) "ZachMacros.ps1"
        Write-ZMLog "Remote launch: downloading to '$tmp'"
        try {
            $content = Invoke-RestMethod $scriptUrl -ErrorAction Stop
            [IO.File]::WriteAllText($tmp, $content, [Text.UTF8Encoding]::new($false))
            Write-ZMLog "Download OK"
        } catch {
            Write-Host "Download failed: $_" -ForegroundColor Red
            Write-ZMLog "Download FAILED: $_"
        }
        if (Test-Path $tmp) {
            Start-Process $psExe "-ExecutionPolicy Bypass -STA -NoProfile -File `"$tmp`" -MacroMode" -WindowStyle Hidden
            Write-ZMLog "ZachMacros launched hidden"
        }
    }

    # Now run HabibiModAnalyzer visually in this window (foreground)
...
Write-ZMLog "MacroMode body starting"
...
[Win32]::FreeConsole() | Out-Null
    ...
    $script:activePreset  = "Zach"
    $script:hcKey1        = 0x32
    $script:hcAction2     = "key"
    $script:hcKey2        = 0x33
    $script:ancSlotAction  = "key"
    $script:ancSlot       = 0x39
    $script:ancBackAction = "key"
    $script:ancBackKey    = 0x35
    $script:crystalAction = "mouse"
    $script:crystalKey    = 0x00
    ...
    function New-ZachForm {
        ...
        if ($script:activePreset -eq "Zach") {
        ...
        if ($script:activePreset -eq "Other Guy") {
        ...
        $wanBtn.Add_Click({
            $script:activePreset  = "Other Guy"
            $script:hcKey1        = 0x32
            $script:hcAction2     = "key"
            $script:hcKey2        = 0x33
            $script:ancSlotAction  = "mouse"
            $script:ancSlot       = 0x00
            $script:ancBackAction = "key"
            $script:ancBackKey    = 0x35
            $script:crystalAction = "key"
            $script:crystalKey    = 0x58
            $ac = [System.Drawing.ColorTranslator]::FromHtml("#cba6f7")
            $this.BackColor = [System.Drawing.ColorTranslator]::FromHtml("#313244")
            $this.ForeColor = $ac
            $this.FlatAppearance.BorderColor = $ac
        ...
        foreach ($section in $macroSections) {
            ...
            foreach ($group in $section.DelayGroups) {
                ...
                foreach ($item in $group.Items) {
                    ...
                    $tbBorder.Size        = New-Object System.Drawing.Size($nudW, 26)
                    $tbBorder.Location    = New-Object System.Drawing.Point(($contentW - $nudW - 8), ($iy + 1))
                    $tbBorder.BackColor   = $surface0

                    $tbInner = New-Object System.Windows.Forms.Panel
                    $tbInner.Size         = New-Object System.Drawing.Size(($nudW - 2), 24)
                    $tbInner.Location     = New-Object System.Drawing.Point(1, 1)
                    $tbInner.BackColor    = $baseBg

                    $nud = New-Object System.Windows.Forms.TextBox
                    ...
                    $nud.Size        = New-Object System.Drawing.Size(($nudW - 10), 20)
                    $nud.Location    = New-Object System.Drawing.Point(4, 3)
                    $nud.Text        = $script:delays[$item.Key]
                    $nud.Tag         = $item.Key
                    $nud.Add_TextChanged({
                        if ([int]::TryParse($this.Text, [ref]$null)) {
                            $script:delays[$this.Tag] = [int]$this.Text
                        } elseif ($this.Text -ne "") {
                            $this.Text = $script:delays[$this.Tag]
                        }
                    })

                    $tbInner.Controls.Add($nud)
                    $tbBorder.Controls.Add($tbInner)
                    $card.Controls.Add($tbBorder)
                    $iy += 32
                }
                $iy += 6
            }
    ...
    $exit = New-Object System.Windows.Forms.ToolStripMenuItem("Exit ZachMacros")
    $exit.Add_Click({
        $timer.Stop()
    ...
    Write-ZMLog "Creating form"
    $script:guiForm = New-ZachForm
    $script:guiForm.Show()
    [Win32]::ShowWindow($script:guiForm.Handle, 5)
    $script:guiForm.Activate()
    Write-ZMLog "GUI visible, entering event loop"

    $appCtx = New-Object System.Windows.Forms.ApplicationContext
    [System.Windows.Forms.Application]::Run($appCtx)

} catch {
    Write-ZMLog "CRASH: $($_.Exception.Message) | $($_.ScriptStackTrace)"
    try {
        Add-Type -AssemblyName System.Windows.Forms
        [System.Windows.Forms.MessageBox]::Show("ZachMacros crashed:`n`n$($_.Exception.Message)`n`n$($_.ScriptStackTrace)", 'ZachMacros Error', 'OK', 'Error') | Out-Null
    } catch {
        Write-Host "ZachMacros error: $_" -ForegroundColor Red
    }
}
